...
apis = ['EVP_CIPHER_CTX_new', 'EVP_CIPHER_CTX_cleanup', 'EVP_BytesToKey', 'EVP_EncryptInit_ex', 'EVP_DecryptInit_ex', 'EVP_EncryptUpdate','EVP_EncryptFinal_ex','EVP_DecryptUpdate','EVP_DecryptFinal_ex','EVP_CIPHER_CTX_free', 'aes_ecb_cipher', 'EVP_CipherInit','EVP_CipherUpdate','EVP_CipherFinal']
        ...
        def parse_args(self):
            ...
            try:
                while(line != ''):
                    ...
                    gdb.execute("set logging on")
                    if 'salt' or 'key' or 'iv' not in arg:
                        gdb.execute(s)
                    gdb.execute("set logging off")
                    gdb.execute("set logging file log.txt")
